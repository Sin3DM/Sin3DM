...
def get_activations(files, model, batch_size=1, dims=64,
    ...
    for i in range(n_batches):
        ...
        pred = model(batch)[0]
        mask = F.interpolate(mask, size=pred.shape[-2:]) > 0.5
        ...
        # only use the masked pixels (foreground)
        # pred_arr = pred.permute(0, 2, 3, 1)[mask.permute(0, 2, 3, 1).squeeze(-1)].detach().cpu().numpy()
...
def calculate_activation_statistics(files, model, batch_size=1,
                                    dims=64, device="cuda:0", verbose=False):
...
def calculate_sifid_given_paths(paths1, paths2, batch_size, cuda, dims, suffix, model=None):
    ...
    for i in range(len(paths2)):
        m1, s1 = calculate_activation_statistics([paths1[i]], model, batch_size, dims, cuda)
        m2, s2 = calculate_activation_statistics([paths2[i]], model, batch_size, dims, cuda)
        fid_values.append(calculate_frechet_distance(m1, s1, m2, s2))
        file_num1 = paths1[i].name
        file_num2 = paths2[i].name
...
@torch.no_grad()
def calculate_multiview_sifid_given_paths(gen_render_dirs, ref_render_dir, batch_size=1, device="cuda:0", dims=64):
    ...
    sifid_list = []
    for i in tqdm(range(n_views), desc='Calculating multi-view SIFID'):
        ref_render_path = os.path.join(ref_render_dir, f'{i:03d}.png')
        gen_render_paths = [os.path.join(gen_render_dir, f'{i:03d}.png') for gen_render_dir in gen_render_dirs]
        assert len(gen_render_paths) > 0

        mu_ref, sigma_ref = calculate_activation_statistics([ref_render_path], model, batch_size, dims, device)

        sifid_view_list = []
        for gen_path in gen_render_paths:
            mu_gen, sigma_gen = calculate_activation_statistics([gen_path], model, batch_size, dims, device)
            sifid = calculate_frechet_distance(mu_ref, sigma_ref, mu_gen, sigma_gen)
            sifid_view_list.append(sifid)
        sifid_view = np.mean(sifid_view_list)
        sifid_list.append(sifid_view)
    sifid_avg = np.mean(sifid_list)
    return {f"mv_sifid_dim{dims}": sifid_avg}
